...
public class ProductMasterSearchServiceImpl implements
	...
	public String findProductByStyleAndColor(String styleNumber,
		...
		return productMasterFeedDao.findProductByStyleAndColor(styleNumber,
				colorCode);
	}
	...
	public String findProductByStyle(String styleNumber, String field)
		...
		return productMasterFeedDao.findProductByStyle(styleNumber,
				fieldsToInclude);
	}

	@Override
	public String findProductByFields(String globalSearchFields,
			String fieldsToInclude) throws Exception {

		String globalFields = globalSearchFields.replaceAll("(\\{|\\})", "");
		String[] searchFields = globalFields.split("=");
		String response = null;

		// check if array size is two since the input would be
		// q={styleNumber=12345,12346}
		...
		LOG.debug("Global search for field : " + columnName + " and value :"
				+ columnValue);
		...
		List<String> columnsToInclude = new ArrayList<String>();
		...
		// If the provided input simply contains colorCode/sizeCode then append
		// appropriate column hierarchy based on DB
		...
		if (StringUtils.isNotBlank(columnName) && columnValues != null) {
			response = productMasterFeedDao.findProductByFields(columnName,
	...
	@Override
	public String globalSearch(String searchField, String fieldsToInclude)
			throws Exception {
		List<String> columnsToInclude = new ArrayList<String>();
		// check if include fields are present
		if (StringUtils.isNotBlank(fieldsToInclude)) {
	...
	public String findProductByStyleColorAndSize(String styleNumber,
			String colorCode, String size) throws Exception {
		return productMasterFeedDao.findProductByStyleColorAndSize(styleNumber,
				colorCode, size);
	}
	...
	public String findProductSizesByStyleAndColor(String styleNumber,
			String colorCode) throws Exception {
		return productMasterFeedDao.findProductSizesByStyleAndColor(
				styleNumber, colorCode);
	}

	@Override
	public String findProducts(String globalSearchFields,
			String fieldsToInclude, String isSearchFieldPresent)
			throws Exception {
		String response = "";
		if (StringUtils.isNotBlank(globalSearchFields)) {
			...
			case "true":
				response = findProductByFields(globalSearchFields,
						fieldsToInclude);
				break;
			default:
